...
extension Array: ABIEncodable where Element: ABIEncodable {
    ...
    public func abiEncodeDynamic() -> String? {
        ...
        if self is [Web3Tupple] {
            var nextStart = 0
            for (i,item) in values.enumerated() {
                let previousLength: Int = i == 0 ? 0 : nextStart
                
                nextStart += item.count / 2
                
                let margin = (self.count * 32) + previousLength
                prefix += String(format:"%02X", margin).paddingLeft(toLength: 64, withPad: "0")

            }
        }
        
        return length + prefix + values.joined()
    }
}
...
extension Web3Tupple: ABIEncodable {
    public func abiEncode(dynamic: Bool) -> String? {
        let addressHex = self.address.hex(eip55: false).replacingOccurrences(of: "0x", with: "")
        let row1 = addressHex.paddingLeft(toLength: 64, withPad: "0")
        let row2 = "40".paddingLeft(toLength: 64, withPad: "0")
        let row3 = String(format:"%02X", (self.callData.utf8.count) / 2).paddingLeft(toLength: 64, withPad: "0")
        let row4 = self.callData
        return row1 + row2 + row3 + row4
    } 
}
 
extension Web3Tupple: SolidityTypeRepresentable {
    public static var solidityType: SolidityType {
        return .string
    }
}
