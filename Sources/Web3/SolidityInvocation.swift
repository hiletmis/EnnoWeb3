...
public protocol SolidityInvocation {
    ...
    /// Read data from the blockchain. Only available for constant functions.
    func callRawResponse(block: EthereumQuantityTag, completion: @escaping (EthereumData?, Error?) -> Void)
...
public struct SolidityReadInvocation: SolidityInvocation {
    ...
    public init(method: SolidityFunction, parameters: [SolidityWrappedValue], handler: SolidityFunctionHandler) {
        self.method = method
    ...
    public func callRawResponse(block: EthereumQuantityTag = .latest, completion: @escaping (EthereumData?, Error?) -> Void) {
        guard handler.address != nil else {
            completion(nil, InvocationError.contractNotDeployed)
            return
        }
        guard let callRawResponse = createCall() else {
            completion(nil, InvocationError.encodingError)
            return
        }
        let outputs = method.outputs ?? []
        handler.callRawResponse(callRawResponse, outputs: outputs, block: block, completion: completion)
    }
...
public extension SolidityInvocation {
    ...
    func callRawResponse(completion: @escaping (EthereumData?, Error?) -> Void) {
        self.callRawResponse(block: .latest, completion: completion)
    }
