...
public protocol ERC20Contract: EthereumContract {
    ...
    func permit(owner: EthereumAddress, spender: EthereumAddress, value: BigUInt, deadline: BigUInt, v: UInt8, r: Bytes, s: Bytes) -> SolidityInvocation
}
...
public extension ERC20Contract {
    ...
    func permit(owner: EthereumAddress, spender: EthereumAddress, value: BigUInt, deadline: BigUInt, v: UInt8, r: Bytes, s: Bytes) -> SolidityInvocation {
        let inputs = [
            ...
            SolidityFunctionParameter(name: "_spender", type: .address),
            SolidityFunctionParameter(name: "_value", type: .uint256),
            SolidityFunctionParameter(name: "_deadline", type: .uint256),
            SolidityFunctionParameter(name: "_v", type: .uint8),
            SolidityFunctionParameter(name: "_r", type: .bytes(length: 32)),
            SolidityFunctionParameter(name: "_s", type: .bytes(length: 32))
        ]
        let method = SolidityNonPayableFunction(name: "permit", inputs: inputs, handler: self)
        return method.invoke(owner, spender, value, deadline, v, r, s)
    }
