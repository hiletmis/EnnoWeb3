...
extension EthereumContract {
    ...
    public func callRawResponse(_ call: EthereumCall, outputs: [SolidityFunctionParameter], block: EthereumQuantityTag = .latest, completion: @escaping (EthereumData?, Error?) -> Void) {
        eth.call(call: call, block: block) { response in
            switch response.status {
            case .success(let data):
                completion(data, nil)
